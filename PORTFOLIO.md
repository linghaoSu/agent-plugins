...
| `idea-to-ship` | Active | linghao | linghao | End-to-end workflow from requirements through architecture, roadmap, implementation, review, and tests. | Global release checks; `bash tests/idea-to-ship-eval-fixtures.sh` for skill contract changes. | Treat as release-critical because it drives portfolio planning. Preserve story/test traceability and runtime-aware review routing. |
...
| `secret-scanner` | Active | linghao | linghao | Detects leaked credentials in staged, working, recent, ranged, or full-repo scopes. | Global release checks; `python3 secret-scanner/scripts/scan.py --mode all --format json`. | Release-critical. `ITS-ROADMAP-007` owns the decision to promote scanning further into the release gate or hooks. |
| `skill-stats` | Experimental | linghao | linghao | Tracks skill invocations via PostToolUse and summarizes local usage. | Global release checks; `bash -n skill-stats/scripts/track-skill.sh`; temp-`HOME` smoke for JSONL append. | Stateful hook. Retention/rotation and malformed JSONL recovery are deferred from ITS-ROADMAP-004. |
