...
|---|---|---|---|---|---|---|
| `agent-playbook` | Active | linghao | linghao | Focused repo-memory, context/tool audit, implementation-tournament, commit, and draft PR workflows. | Global release checks; `bash tests/agent-playbook-eval-fixtures.sh` for workflow contract changes. | Review when source-practice guidance changes or skills duplicate `harness-engineering` / `idea-to-ship`. |
| `antifragile` | Active | linghao | linghao | Provides one read-only `antifragile-audit` with explicit `agent` and `system` scopes. | Global release checks. | Keep the two scope checklists distinct while sharing output and safety contracts. |
