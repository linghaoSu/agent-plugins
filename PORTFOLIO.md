...
| `idea-to-ship` | Active | linghao | linghao | End-to-end workflow from requirements and commercialization through UI design, architecture, roadmap, implementation, review, and tests. | Global release checks; `bash tests/idea-to-ship-eval-fixtures.sh` for skill contract changes. | Treat as release-critical because it drives portfolio planning. Preserve commercialization evidence, UI design contracts, story/test traceability, and runtime-aware review routing. |
...
| `skill-stats` | Experimental | linghao | linghao | Tracks skill invocations via PostToolUse and exposes a conversation-only read of local usage statistics. | Global release checks; `bash -n skill-stats/scripts/track-skill.sh`; temp-`HOME` smoke for JSONL append. | Stateful hook writes JSONL, but the user-facing skill is read-only. Retention/rotation and malformed JSONL recovery are deferred from ITS-ROADMAP-004. |
